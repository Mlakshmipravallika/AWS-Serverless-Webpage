...
def lambda_handler(event, context):
    ...
    if response_dict['status'] == 'Success':
        ...
        response_db = dynamodb.put_item(
            TableName = 'my-secret-converter',
            Item = {
                'id': {'N': str(random.randint(1,100000))},
                'name': {'S': response_dict['name']},
                'option': {'S': response_dict['code']},
                'result': {'S': response_dict['encript_string']}
            }
            )
    return {
